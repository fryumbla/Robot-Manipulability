...
class WorkspaceSpreader:
    ...
    def around(self, position):
        for dx in [0.0, -self.dx]:
            for dy in [0.0, -self.dy, self.dy]:
                for dz in [0.0,self.dz]:
                    new_p = (position[0] + dx, position[1] + dy, position[2] +dz)
                    if new_p != position:
                        if new_p[2] <= 0.2:
                            if new_p[0] >= 0.2:
                                if new_p[1] <= 0.2 and new_p[1] >= -0.2:
                                    yield new_p 
...
class Recorder:
    def __init__(self):
        ...
        _init_ws_pose.position.x = 0.3
        _init_ws_pose.position.y = 0
        _init_ws_pose.position.z = 0.1
