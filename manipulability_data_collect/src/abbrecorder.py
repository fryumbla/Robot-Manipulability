...
class MoveGroupPythonInteface(object):
    ...
    def go_to_pose_goal(self, position, quat):
        ...
        # print tf.transformations.euler_from_quaternion([quat.x,quat.y,quat.z,quat.w])
        # print self.move_group.get_current_rpy()

        quat_goal = tf.transformations.euler_from_quaternion([quat.x,quat.y,quat.z,quat.w])
        pose_goal.orientation.x = abs(quat_goal[0])
        pose_goal.orientation.y = abs(quat_goal[1])
        pose_goal.orientation.z = abs(quat_goal[2])
        ...
        current_pose.position = self.move_group.get_current_pose().pose.position
        quat_current = self.move_group.get_current_rpy()
        current_pose.orientation.x = abs(quat_current[0])
        current_pose.orientation.y = abs(quat_current[1])
        current_pose.orientation.z = abs(quat_current[2])
        current_pose.orientation.w = 0
        
        # print(pose_goal)
        # print(current_pose)

        return all_close(pose_goal, current_pose, 0.02)# chequear los angulos
...
class Recorder:
    def __init__(self):
        ...
        _frame_id = "table"
        _group_name = "irb120"
        _moveit_eef = "tool0"  # default

        _init_ws_pose = geometry_msgs.msg.Pose()
        _init_ws_pose.position.x = 0.4
        _init_ws_pose.position.y = 0
        _init_ws_pose.position.z = 0.2
        # The interest_eef pose relative to moveit_eef
        self.interest_eef_offset = geometry_msgs.msg.Pose()
        self.interest_eef_offset.position.x = -0.12
        self.interest_eef_offset.position.y = 0.025 # andcho del connector
        # self.interest_eef_offset.position.z = 0.172
        self.interest_eef_offset.position.z = 0.1855
        # self.interest_eef_offset.orientation.w = 1
        ...
        self.tg_marker_x.scale.z = 0.015  # arrow height
        ...
        self.tg_marker_z.scale.z = 0.015  # arrow height
        ...
        self.m_marker.scale.z = 0.004  # arrow height
        # ID_manip: 300 ~
    ...
    def try_target(self, eef_pose, tcp_pose, count):
        ...
        try:
            ...
            self.tg_marker_z.pose.orientation = self.quat_msg_rotation(eef_pose.orientation, np.radians(180), (0, 1, 0))
            ...
            this_marker.pose.orientation = self.quat_msg_rotation(
                eef_pose.orientation, np.radians(180), (0, 1, 0)
            )
            if success:
    ...
    def main(self):
        ...
        #         ),
        #     )
        #     for Cr in range(-90, 91, 45)#here set the angle test 15 degre or 45 degre 5 points 
        # ]
        ...
        approach_set = [
            ...
            for Cr in range(-90, 91, 15)#here set the angle test 15 degre or 45 degre 5 points 
        ]

        # print(approach_set)

        ymd_hms = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        file_name = "abbrobot_manip_%s.csv" % ymd_hms
        ...
        with open(os.path.join(directory, file_name), "w") as csvfile:
            ...
            while self.ws.is_available():
                ...
                for Cr, quat in approach_set:
                    ...
                    manipulability, joint_values = self.try_target(corrected_p, p, count)
