    ...
    def around(self, position):
        for dx in [0.0, self.dx]:
            for dy in [0.0, self.dy, -self.dy]:
                ...
                new_p = (position[0] + dx, position[1] + dy, position[2])
                if new_p != position:
                    # if new_p[2] <= 0.3 and new_p[2] >= 0.1:
                    if new_p[0] <= 0.4:
                        if new_p[1] <= 0.1 and new_p[1] >= -0.1:
                            yield new_p
...
class Recorder:
    def __init__(self):
        ...
        self.interest_eef_offset.position.z = 0.23
    ...
    def try_target(self, eef_pose, tcp_pose, count):
        ...
        try:
            ...
            self.tg_marker_z.pose.orientation = self.quat_msg_rotation(eef_pose.orientation, np.radians(-90), (0, 1, 0))
    ...
    def main(self):
        approach_set = [
            ...
            for Cr in range(-90, 91, 45) #el tercer numero 45 lo cambie por 15
        ]

        # print(approach_set)

        ymd_hms = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        file_name = "indyrobot_manip_%s.csv" % ymd_hms
